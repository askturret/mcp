...
// docs/adr/ADR-024-output-must-vary-with-the-fact.md draws exactly this line —
// "is the number the property, or a by-product of it?" — and rules that pinned
// counts are not automatically defects.
checkIncludes(
...
// which guarantees it reddens again and makes the bump reflexive.
// docs/adr/ADR-024-output-must-vary-with-the-fact.md names that the tally trap
// and cites this exact line as its example. It went red for
...
// as a pass — docs/adr/ADR-024-output-must-vary-with-the-fact.md — so the
// population has to be non-empty for the assertion above to mean anything.
checkTrue(
