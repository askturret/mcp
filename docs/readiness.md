...
|---|---|---|---|---|---|
| 1 | Canonical model supports OpenAPI, explicit definitions, and two framework adapters without source-specific fields | ✅ met | [Compile-time guards forbidding source-specific fields](../packages/core/src/__tests__/types.test.ts) — two `@ts-expect-error` assertions checked by `tsc`, which CI runs as `npm run build -w packages/core` in the `test-core` job. The compiler is the runner: if the canonical shape ever admits a source-native field, or a raw `Error` becomes a valid `OperationError`, the directive goes unused, `tsc` raises `TS2578` and the build fails. Being a compiler check, it holds regardless of whether jest collects the file ([#216](https://github.com/askturret/mcp/issues/216)). Scope: the guard closes the top-level `OperationDefinition` shape; `executor.config` and `annotations` are deliberately open records and are not constrained by it. [Conformance suite](../packages/adapter-conformance/src/__tests__/conformance.test.ts) green for Express + Fastify | Architecture | 2026-08-25 |
...
| 6 | Non-idempotent writes are never automatically retried | ✅ met | [`OUTCOME_UNKNOWN` no-retry test](../packages/core/src/retry/__tests__/dispatcher-retry.test.ts); [Exhaustive retry-matrix test](../packages/core/src/__tests__/idempotent-retryable-fuzz.test.ts) — drives the real `decideRetry` and `isRetryEligible` across all 14 error codes × all 16 effect combinations (224 cases): `OUTCOME_UNKNOWN` never retries under any combination, a non-idempotent mutating operation never retries under any code, and a retry is only ever returned for a transient code the effects matrix permits | Core team | 2026-08-24 |
...
| 12 | A new source or executor can be added as a plugin without modifying core control flow | ✅ met | [`registerSource` / `registerExecutor` on `PluginContext`](../packages/core/src/plugin/types.ts), capability-gated in [`host.ts`](../packages/core/src/plugin/host.ts); [plugin tests](../packages/core/src/plugin/__tests__/plugin.test.ts) register a real `OperationSource` through a plugin and assert it is usable as compiler input. Two runnable examples show the adopter-facing lifecycle: [an exporter plugin](../examples/plugin-otel-exporter/) and [a source plugin](../examples/plugin-source/) that compiles its contributed operations through the real compiler. Both are EXECUTED by the `test-integrity` job, not merely built, so a broken plugin seam fails CI rather than leaving a stale example (#270). | Architecture | 2026-08-25 |
